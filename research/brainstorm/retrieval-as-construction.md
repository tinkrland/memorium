...
this is not a hypothetical concern for this project specifically — the substrate pilot (see [../../pilot/readme.md](../../pilot/readme.md)) found exactly this empirically while canonicalizing a messy field-name corpus: canonicalization revealed structure that looked meaningfully higher (metrics improved) while the actual finding underneath was closer to "cleaning up field names surfaced connections that were always there" mixed with "the cleanup itself manufactured some of the appearance of connectedness." the notebook theorized the failure mode and the pilot hit it in the data, independently, before either had a name for it. theory and practice converging on the same trap from opposite directions is about as strong a signal as either gets.
